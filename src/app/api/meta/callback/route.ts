...
export async function GET(request: NextRequest) {
  ...
  if (error || !code) {
    return new Response(JSON.stringify({ error, code: null, message: 'No code received' }), {
  ...
  try {
    ...
    // Return raw response so we can see what Instagram says
    return new Response(JSON.stringify({ tokenData, REDIRECT_URI, META_APP_ID: META_APP_ID ? 'set' : 'missing', META_APP_SECRET: META_APP_SECRET ? 'set' : 'missing' }), {
      headers: { 'Content-Type': 'application/json' }
    })
  } catch (err: any) {
